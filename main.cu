...
int main(void) {
  ...
  cudaMalloc((void **)&aft_device, sizeof(BefAft));
  cudaMalloc((void **)&bef_device, sizeof(BefAft));
  cudaMalloc((void **)&ma_device, sizeof(MedArr));
  cudaMalloc((void **)&dif_device, sizeof(Diff));
  cudaMalloc((void **)&ran_device, sizeof(Range));
  cudaMalloc((void **)&ip_device, sizeof(Inpaluse));
  ...
  initBefAft(&bef_host, ran_host);
  initBefAft(&aft_host, ran_host);
  
  initMedArr(&ma_host, ran_host.sr);
  initInpalse(&ip_host, ran_host.sr, pml, ip_host.mode, ip_host.in.x, ip_host.in.y, ip_host.in.z, ip_host.freq);//
  printf("in:%d,%d,%d\n", ip_host.in.x, ip_host.in.y, ip_host.in.z);
  ...
  for (int t = 0; t < tmax; t++) {
    Vel(&aft_device, &bef_device, ma_device, dif_device, ran_host.vr);
    Sig(&aft_device, &bef_device, ma_device, dif_device, ran_host.sr, ip_host, t);
    Tau(&aft_device, &bef_device, ma_device, dif_device, ran_host.tr);
    // 加速度算出＆書き込み
